...
public class Rifle : MonoBehaviour
{
    ...
    public Sprite idleRifle;
    public Sprite shotRifle;
    ...
    private void Update()
    {
       ...
       if(Input.GetKeyDown("mouse 0") == true && isReloading == false)
            isShot = true;
        ...
        if (isShot == true && ammoClipLeft > 0 && isReloading == false)
        {
            ...
            if (Physics.Raycast(ray, out hit, rifleRange))
            {
                if (hit.collider.CompareTag("Enemy"))
                {
                    ...
                    hit.collider.gameObject.SendMessage("AddDamage", rifleDamage, SendMessageOptions.DontRequireReceiver);
                }
                else
                {
    ...
    IEnumerator Shot()
    {
        GetComponent<SpriteRenderer>().sprite = shotRifle;
        //animation time!  rifles shoot fast fast
        yield return new WaitForSeconds(0.1f);
        GetComponent<SpriteRenderer>().sprite = idleRifle;
